...
def getRelatedURLs(url):
	...
	html = ""
	try:
		response = requests.get(url, headers=hdr, cookies=cookielib.CookieJar())
		response = requests.get(url, headers=hdr, cookies=cookielib.CookieJar())
...
with open('inputSites.txt', 'r') as input, open('articlesSummary.txt', 'w') as summary, io.open('articles.txt', 'w', encoding="utf-8") as output:
	for main in input:
		main = main.strip()
		cat = main.split(",")[1]
		fact = main.split(",")[2]
		...
		while(len(listlinks) < MAX_ARTICLES and len(toVisit) > 0):
			...
			if (url == main) or (url != main and len(content) > MIN_ARTICLE_LENGTH):
				if len(content) > MIN_ARTICLE_LENGTH:
					data = {"main": main, "category" : cat, "fact" : fact, "bias" : bias, "url" : url, \
							"title" : title, "date" : date, "authors" : authors, "content" : content}
		...
		summary.write(main+" "+str(len(listlinks))+"\n")
